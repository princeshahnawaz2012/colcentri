...
Feature: Show archived tasks in task lists
  ...
  As a Colcentric developer
  I want users to see archived tasks in task lists quickly and easily

  Background: 
    Given a project exists with name: "Market Colcentric"
    And a confirmed user exists with login: "balint"
    And "balint" is in the project called "Market Colcentric"
    And a task list exists with name: "This week"
    And the task list called "This week" belongs to the project called "Market Colcentric"
    And the following tasks with associations exist:
      | name                   | task_list | project        |
      | Tell my friends        | This week | Market Colcentric |
      | Tell the tech bloggers | This week | Market Colcentric |
    And the task called "Tell my friends" is resolved
    And I am logged in as @balint

  Scenario: See archived tasks
    When I go to the "Market Colcentric" tasks page
  ...
  Scenario: No archived tasks in a task list
    ...
    And the task list called "Next month" belongs to the project called "Market Colcentric"
    And the following tasks with associations exists:
      | name                         | task_list  | project        |
      | Post on Digg and Hacker News | Next month | Market Colcentric |
    When I go to the "Market Colcentric" tasks page
