...
Feature: Converting a conversation to a task
  ...
  Scenario: Converting a simple conversation on the overview page and specifying additional task attributes
    ...
    | pablos | pablo@colcentric.com        | Pablo      | Villalba  |
    | saimon | saimon@colcentric.com       | Saimon     | Moore     |
    And I am in the project called "Colcentric"
    And all the users are in the project with name: "Colcentric"
    And the following task lists with associations exist:
      | name         | project |
      | Next release | Colcentric |
      | Bugfixes     | Colcentric |
    Given I started a simple conversation in the "Colcentric" project
    When I go to the page of the "Colcentric" project
    And I click the conversation's comment box
    And I follow "Convert to task"
    And I wait for 2 seconds
