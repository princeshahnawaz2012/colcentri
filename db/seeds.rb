...
def seed_data
  users = [%w(Frank Kramer frank),
    ...
    user = User.find_by_login(u) || User.create!(:login => u,
                        ...
                        :notify_tasks => false,
                        :email => "example_#{a}@colcentric.com")
    user.activate!
    user
  end
  ...
  earthworks.status_update(marco, "I uploaded the images to the Colcentric Files tab on Thursday. Let me know if/when you’ve reviewed them and how they look.", { :due_on => (fake_time + 3.day), :assigned_user => corrina})

  earthworks.make_task(frank, "Contact area businesses for banner exchange")
  earthworks.status_update(frank, "Please contact Green Earth Cafe, Nellie’s Tacos, Fenton’s, ROOZ, and Cato’s about a banner exchange with EarthworksYoga.com. Verbiage for your email can be found here in Colcentric on the Pages tab.", {:due_on => (fake_time + 1.day), :assigned_user => maya})
