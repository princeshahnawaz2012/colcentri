    ...
    describe "emails" do
      ...
      it "handles encoded headers" do
        post :create, default_params(
          :to => "=?ISO-8859-1?Q?Moo?= <#{@project.permalink}@#{Colcentric.config.smtp_settings[:domain]}>\n",
          :text => "Hello there"
        )
      ...
      context "the bounce system" do
        ...
        it "should raise (200 for sendgrid) and create a bounce message if a project is not found" do
          options = @options.merge!(:to => "#{@project.permalink}+task+#{rand(1000) + 1000}@#{Colcentric.config.smtp_settings[:domain]}")
          check_bounce_message(options) do
            post :create, options
          end
          response.response_code.should == 200
        ...
        it "should raise (200 for sendgrid) and create a bounce message if a conversation is not found" do
          options = @options.merge(:to => "#{@project.permalink}+conversation+#{rand(1000) + 1000}@#{Colcentric.config.smtp_settings[:domain]}")
          check_bounce_message(options) do
            post :create, options
          end
          response.response_code.should == 200
        ...
        it "should raise (200 for sendgrid) and create a bounce message for invalid target" do
          address = "#{@project.permalink}+tasknuevo@#{Colcentric.config.smtp_settings[:domain]}"
      ...
      def post_options(to, subject, body, attachments = true)
         {
           ...
           :to => "#{to}@#{Colcentric.config.smtp_settings[:domain]}",
           :text => body,
           :subject => subject,
      ...
      def default_params(more = {})
        { :hook_name => 'email',
          :from => @project.user.email,
          :to => "#{@project.permalink}@#{Colcentric.config.smtp_settings[:domain]}",
          :text => "Nothing to say",
