...
describe Emailer do 
  describe 'incoming emails' do 
    ...
    context 'an email to create a new task' do
      before do
        @email_template.to = "#{@project.permalink}+task@#{Colcentric.config.smtp_settings[:domain]}"
    ...
    it "should not assign or change the status of the task with no action" do
      @email_template.to = "#{@project.permalink}+task+#{@task.id}@#{Colcentric.config.smtp_settings[:domain]}"
      @email_template.body = "#\nWe did some stuff"
      Emailer.receive(@email_template.to_s)
    ...
    it "should post a comment to a project if no subject is given" do
      @email_template.to = "#{@project.permalink}@#{Colcentric.config.smtp_settings[:domain]}"
    ...
    it "should post a comment to a conversation" do
      @email_template.to = "#{@project.permalink}+conversation+#{@conversation.id}@#{Colcentric.config.smtp_settings[:domain]}"
    ...
    it "should create a conversation with the subject as title and the body as comment" do
      @email_template.to = "#{@project.permalink}+conversation@#{Colcentric.config.smtp_settings[:domain]}"
    ...
    context "should raise an error when" do
      ...
      it "the specified project does not exist" do
        @email_template.to = "random_project+tasks@#{Colcentric.config.smtp_settings[:domain]}"
      ...
      it "the specified conversation does not exist" do
        @email_template.to = "#{@project.permalink}+conversation+#{rand(1000) + 1000}@#{Colcentric.config.smtp_settings[:domain]}"
      ...
      it "the specified task does not exist" do
        @email_template.to = "#{@project.permalink}+task+#{rand(1000) + 1000}@#{Colcentric.config.smtp_settings[:domain]}"
