require 'spec_helper'
describe ColcentricData do
  before do
    Colcentric.config.delay_data_processing = false
  end
  ...
  describe "unserialize" do
    it "should unserialize data" do
      ...
      ColcentricData.new.tap{|d| d.data = data }.unserialize({}, {:create_organizations => true})
    ...
    it "should map existing data where specified" do
      ...
      ColcentricData.new.tap{|d| d.data = data; d.user = user }.unserialize(
    ...
    it "should create users when specified" do
      ...
      ColcentricData.new.tap{|d| d.data = data }.unserialize({}, {:create_users => true, :create_organizations => true})
    ...
    it "should unserialize a basecamp dump" do
      ...
      ColcentricData.new.tap{|d| d.service = 'basecamp'; d.data = data }.unserialize({}, {:create_users => true, :create_organizations => true})
  ...
  describe "serialize" do
    it "should serialize data" do
      make_the_teambox_dump
      encoded_data = ActiveSupport::JSON.encode ColcentricData.new.serialize(Organization.all, Project.all, User.all)
  ...
  describe "do_export" do
    it "should only serialize projects from organizations the user is an admin of" do
      ...
      user = Factory(:user)
      dump = ColcentricData.new.tap{|d|d.type_name='export';d.user=user}
  ...
  describe "do_import" do
    it "should import projects into the target organization" do
      ...
      organization.add_member(user, Membership::ROLES[:admin])
      dump = ColcentricData.new.tap{|d|d.type_name='import';d.service='teambox';d.user=user; d.save}
    ...
    it "should still import projects into the target organization after a DJ save" do
      ...
      dump.save.should == true
      dump = ColcentricData.find_by_id(dump.id)
    ...
    it "should not import projects into an organization the user is not an admin of" do
      ...
      dump = ColcentricData.new.tap{|d|d.type_name='import';d.service='teambox';d.user=user}
      dump.data = @teambox_dump
      dump.target_organization = organization.permalink
      dump.user_map = user_map
      dump.status_name = :mapping
  ...
  describe "import_from_file" do
    it "should import data from a file" do
      ColcentricData.import_from_file("#{Rails.root}/spec/fixtures/teamboxdump.json", {}, {:create_users => true, :create_organizations => true})
  ...
  describe "export_to_file" do
    it "should export data to a file" do
      make_the_teambox_dump
      ColcentricData.export_to_file(Project.all, User.all, Organization.all, "#{Rails.root}/tmp/test-export.json")
    end
  end
