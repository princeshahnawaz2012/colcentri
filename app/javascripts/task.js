...
document.on('ajax:failure', 'form.new_task.app_form', function(e, form) {
    ...
    var name_field = form.task_name;
    var description_field = form.task_comments_attributes_0_body
    var blank_message_error = document.getElementById("name_blank")
    var blank_description_error = document.getElementById("description_blank")
        if (!(name_field.getValue()))   {
                                        name_field.up(0).addClassName("field_with_errors");
                                        blank_message_error.setAttribute("style", "inline");
                                        }
        if (!(description_field.getValue()))    {
                                                description_field.up(0).addClassName("field_with_errors");
                                                blank_description_error.setAttribute("style", "inline");
                                                }
        ...
        if (typeof _people[project_id] == "object") {
          _people[project_id].each(function(person) {
            if (!select.select('[value=' + person[0] + ']').any()) {
              var option = new Element('option', { 'value': person[0] }).insert(person[1])
              // var option = new Element('option', { 'value': person[0] }).insert(person[2]) for fullname instead of nick
