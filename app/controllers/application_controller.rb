...
class ApplicationController < ActionController::Base
  ...
  private
    ...
    def subscription_done?
      if current_user and current_user.is_active? and not current_user.subscription_active?
    ...
    #Updates the header with the 3-bars
    def refresh_custom_header
      update_page do |page|
        page["custom_header"].reload
      end
    end
    ...
    def signups_enabled?
      Colcentric.config.community || User.count == 0
    end
    ...
    def load_community_organization
      if logged_in? and Colcentric.config.community
