class ResetPasswordsController < ApplicationController
  ...
  def create
    ...
    if @reset_password.save
      flash[:error] = nil
      Emailer.send_with_language :forgot_password, @reset_password.user.locale, @reset_password.id
      ...
      if @reset_password.errors.on(:user)
        ...
        flash[:error] = I18n.t('reset_passwords.create.not_found_html',
                                {:email => @reset_password.email, :support => Colcentric.config.support})
      end
      render :new
    end
  end
  ...
  def reset
    begin
      ...
      flash[:error] = I18n.t('reset_passwords.create.invalid_html', :support => Colcentric.config.support)
  ...
  def update_after_forgetting
    ...
    if @reset_password and @reset_password.valid?
      ...
      flash.now[:notice] = I18n.t('reset_passwords.create.invalid_html', :support => Colcentric.config.support)
      @reset_password = ResetPassword.new
