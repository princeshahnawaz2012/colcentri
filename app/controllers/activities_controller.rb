class ActivitiesController < ApplicationController
  ...
  def show_thread
    # FIXME: insecure!
    #the below line did not work properly when showing comments of a deleted task
    #target = params[:thread_type].constantize.find params[:id]
    #@comments = target.comments
    @comments = Comment.where(:target_type => params[:thread_type], :target_id => params[:id]).order(:created_at)
