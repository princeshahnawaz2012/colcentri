class UsersController < ApplicationController
  ...
  skip_before_filter :confirmed_user?, :subscription_done?, :only => [ :new, :create, :confirm_email, :forgot_password, :reset_password, :login_from_reset_password, :unconfirmed_email ]
  ...
  def show
    ...
    respond_to do |format|
      if @user != @current_user and (!@shares_invited_projects and @projects_shared.empty?) # TODO check for corp version
        format.any(:html, :m) {
  ...
  def create
    ...
    @user.confirmed_user = ((@invitation && @invitation.email == @user.email) or 
                            ...
                            Rails.env.development? or !Colcentric.config.email_confirmation_require)

    if User.count == 0
      @user.admin = true
    elsif Colcentric.config.community
    ...
    if @user && @user.save
      ...
      if membership_community
        Organization.all.first.add_member(current_user, Membership::ROLES[:participant])
      end
      ...
      if not @user.subscription_active?
        ...
        # Can be an invitation to a project or just to Colcentric
  ...
  private
    ...
    def can_users_signup?
      unless @invitation || signups_enabled?
        ...
        return redirect_to Colcentric.config.community ? login_path : root_path
      end
    end

  def show_terms
    #"#{Rails.root}/public/terms_of_use.html"
    render :file => "#{Rails.root}/public/terms_of_use.html"
  end
end
