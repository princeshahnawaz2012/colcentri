class TaskListsController < ApplicationController
  ...
  before_filter :check_project_archived, :only => [:index, :show]
  ...
  def index
    @project_users = []
    people = Person.where(:project_id => @current_project.id)
    people.each do |person|
=begin
      begin
        u = User.find(person.id)
      rescue
        u = -1
      end
=end
      ...
      num_tasks = Task.where("assigned_id = ? and project_id = ? and status > 0 and status < 3 ", person.id, @current_project.id).count
      login_and_tasks = u.login + ' (' + num_tasks.to_s + ')'
      @project_users.push( [login_and_tasks, u.login]  )


    end

    @project_users.insert(0, [t('task_lists.filter.unassigned'), t('task_lists.filter.unassigned')] )
    @project_users.insert(0, [t('task_lists.filter.my_tasks'), t('task_lists.filter.my_tasks') ] )
    @project_users.insert(0, [t('task_lists.filter.anybody'), t('task_lists.filter.anybody')] )
    @project_users.insert(0, [t('task_lists.filter.responsible'), t('task_lists.filter.responsible')] )

    @dates = [t('task_lists.filter.date'),
              t('task_lists.filter.anytime'),
              t('task_lists.filter.late_tasks'),
              t('task_lists.filter.no_date_assigned'),
              t('task_lists.filter.today'),
              t('task_lists.filter.tomorrow'),
              t('task_lists.filter.week'),
              t('task_lists.filter.2weeks'),
              t('task_lists.filter.3weeks'),
              t('task_lists.filter.month')]
    @on_index = true
    respond_to do |f|
      f.any(:html, :m)
      f.rss {
  ...
  def show_filtered
    ...
    @date = params[:date]
    @current_project = Project.find(params[:current_project_id])
    @project_users = []
    people = Person.where(:project_id => params[:current_project_id])
    people.each do |person|
      u = User.find(person.user_id)
      num_tasks = Task.where("assigned_id = ? and project_id = ? and status > 0 and status < 3 ", person.id, @current_project.id).count
      login_and_tasks = u.login + ' (' + num_tasks.to_s + ')'
    ...
    @task_lists = TaskList.where(:project_id => params[:current_project_id])
  ...
  def my_tasks
    ...
    @tasks = Array.new
    #push if the user is the responsible or a participant in the task
    Task.all.each do |t|
      task_marked = false
      people.each do |p|
        if t.assigned_id == p.id
          @tasks.push(t)
          task_marked = true
        end
      end
      @tasks.push(t) if not task_marked and TaskParticipant.where(:task_id => t.id, :user_id => current_user.id).count > 0
    end
