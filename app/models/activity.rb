class Activity < ActiveRecord::Base
  ...
  scope :for_private_messages, :conditions => "target_type = 'Message'"
  scope :in_targets, lambda {|targets| {:conditions => ["target_id IN (?) OR comment_target_id IN (?)", *(Array(targets).collect(&:id)*2)]}}

  scope :latest, :order => 'id DESC', :limit => Colcentric.config.activities_per_page

  scope :in_projects, lambda { |projects| { :conditions => ["project_id IN (?)", Array(projects).collect(&:id) ] } }
  scope :limit_per_page, :limit => Colcentric.config.activities_per_page
  ...
  def self.log(project,target,action,creator_id)
    ...
    @debug_creator = creator_id          #id
    @debug_project = project             #id
    @debug_target = target               #object
    @debug_action = action               #string
   ...
   @debug_creator = @debug_project = @debug_target = @debug_action = nil
  end
