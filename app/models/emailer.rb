class Emailer < ActionMailer::Base
  ...
  class << self
    ...
    def send_with_language(template, language, *args)
      ...
      # Not working properly. Confirm e_mail always in Spanish
    ...
    def from_user(reply_identifier, user)
      unless Colcentric.config.allow_incoming_email and reply_identifier
    ...
    def from_address(recipient = "no-reply", name = "Kinubi")
      domain = Colcentric.config.smtp_settings[:domain]
      address = "#{recipient}@#{domain}"

      if name.blank? or Colcentric.config.smtp_settings[:safe_from]
        address
      else
        %("#{name}" <#{address}>)
      end
  ...
  def trial_reminder(user_id)
    @user = User.find(user_id)
    @url = subscriptions_url
    #@login_link =
    mail(
        :to   => @user.email,
        :subject => I18n.t('emailer.trial.reminder_subject')
    )
  end
  ...
  def trial_expiration(user_id)
    ...
    mail(
        :to => @user.email,
        :subject => I18n.t('emailer.trial.expiration_subject')
    )
  end

  def license_reminder(user_id)
      @user = User.find(user_id)
      @url = subscriptions_url
      mail(
          :to => @user.email,
          :subject => I18n.t('emailer.license.reminder_subject')
      )
    end

  def license_expiration(user_id)
      @user = User.find(user_id)
      @url = subscriptions_url
      mail(
          :to => @user.email,
          :subject => I18n.t('emailer.license.expiration_subject')
      )
  end

  def videoconference_invitation(user_id, video_id)

    @user = User.find(user_id)
    @videoconference = Videoconference.find(video_id)
    @invited_by = User.find(@videoconference.user_id)
    #@url_video = "http://www.google.es"
    #@videoconference.friendly_url = @user.email
    #@videoconference.save
    @url_video = @videoconference.get_personal_session_link(@user.email)

    @url_colcentric = "http://www.colcentric.com"
    mail(
          :to => @user.email,
          :subject => I18n.t('emailer.videoconference.invitation.subject')
      )
  end

  def videoconference_invitation_non_member(email, video_id)

      #@user = User.find(user_id)
      @videoconference = Videoconference.find(video_id)
      @invited_by = User.find(@videoconference.user_id)
      @url_video = @videoconference.get_personal_session_link(email)
      @url_colcentric = "http://www.colcentric.com"
      mail(
            :to => email,
            :subject => I18n.t('emailer.videoconference.invitation.subject')
  ...
  def videoconference_destruction(login_or_email, video_id)
    @user = User.find_by_username_or_email(login_or_email)
    @videoconference = Videoconference.find(video_id)
    @owner = User.find(@videoconference.user_id)
    ...
    mail(
        :to => email,
        :subject => I18n.t('emailer.videoconference.destruction.subject')
    )
  end
  ...
  def notify_export(data_id)
    @data  = ColcentricData.find(data_id)
    @user  = @data.user
    @error = !@data.exported?
    mail(
      :to         => @data.user.email,
      :subject    => @error ? I18n.t('emailer.colcentricdata.export_failed') : I18n.t('emailer.colcentricdata.exported')
    )
  end
  ...
  def notify_import(data_id)
    ...
    mail(
      :to         => @data.user.email,
      :subject    => @error ? I18n.t('emailer.colcentricdata.import_failed') : I18n.t('emailer.colcentricdata.imported')
    )
  end
  ...
  def membership_notification(invitation_id)
    @invitation = Invitation.find_with_deleted(invitation_id)
    @target     = @invitation.target
    @recipient  = @invitation.invited_user
    mail({
      :to            => @invitation.invited_user.email,
      :subject       => I18n.t("emailer.membership_notification.subject",
                               :user => @invitation.user.name, 
                               :target => @invitation.target.name)
    }.merge(
      from_reply_to "#{@invitation.target.permalink}", @invitation.user
    ))
  end
  ...
  def bounce_message(exception_mail, pretty_exception)
    info_url = 'http://help.colcentric.com/faqs/advanced-features/email'
