class Organization < ActiveRecord::Base
  ...
  has_many :members, :through => :memberships, :source => :user, :conditions => {'memberships.role' => Membership::ROLES[:member]}
  ...
  has_attached_file :logo, 
    ...
    :path => (Colcentric.config.amazon_s3 ? "logos/:id/:style.png" : ":rails_root/public/logos/:id/:style.png"),
    :s3_protocol => (Colcentric.config.secure_logins ? 'https' : 'http'),
  ...
  def add_member(user_id, role=Membership::ROLES[:member])
  ...
  protected
    ...
    def ensure_unicity_for_community_version
      if Colcentric.config.community && new_record?
