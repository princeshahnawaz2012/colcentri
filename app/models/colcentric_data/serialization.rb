class ColcentricData
  ...
  def unserialize(object_maps, opts={})
    if service == 'basecamp'
      ...
      unserialize_colcentric({'users' => data['account']['users'],
  ...
  def self.import_from_file(name, user_map, opts={})
    ...
    ColcentricData.new.tap{|d| d.service = opts[:format]||'colcentric'; d.data = data}.unserialize(user_map, opts)
  end
  
  def self.export_to_file(projects, users, organizations, name)
    data = ColcentricData.new.serialize(organizations, projects, users)
