class ColcentricData < ActiveRecord::Base
  include Immortal

  belongs_to :user
  concerned_with :serialization, :attributes, :colcentric, :basecamp
  ...
  def check_map
    @errors.add "service", "Unknown service #{service}" if !['colcentric', 'basecamp'].include?(service)
  ...
  def check_state
    ...
    if type_name == :import
      ...
      when :mapping
        ...
        if Colcentric.config.delay_data_processing
          self.status_name = :pre_processing
          ColcentricDataColcentricData.send_later(:delayed_import, self.id)
        else
          self.status_name = :processing
          do_import
        end
  ...
  def post_check_state
    if type_name == :export
      ...
      ColcentricData.send_later(:delayed_export, self.id) if @dispatch_export
  ...
  def self.delayed_export(data_id)
    ColcentricData.find_by_id(data_id).try(:do_export)
  end
  
  def self.delayed_import(data_id)
    ColcentricData.find_by_id(data_id).try(:do_import)
  end
