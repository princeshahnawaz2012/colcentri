class Task < RoleRecord
  ...
  attr_accessible :name, :assigned_id, :status, :due_on, :comments_attributes, :private, :priority, :start_date, :task_participants

  #validates_presence_of :first_comment
  ...
  before_save :transition_from_new_to_open #, :if => :assigned_id? Now we want open to be the first state always
  ...
  def validate
    errors.add_to_base t('tasks.error.dates.wrong_start_and_end') if (not start_date.nil? and not due_on.nil? and start_date > due_on)
    errors.add_to_base t('tasks.error.participants.participants_without_responsible') if (not task_participants.nil? and task_participants != "" and assigned.id.nil?)
    begin
    ...
    participants.each do |participant|
      user_id = User.find_by_login(participant).id
      errors.add_to_base t('tasks.error.participants.participants_not_in_project') if Person.where(:user_id => user_id, :project_id => project_id).count == 0
    end
  end
  ...
  def track_changes?
    ...
    (updating_user and (status_changed? or assigned_id_changed? or due_on_changed? or priority_changed? or start_date_changed? or task_participants_changed?))
  end
  ...
  def due_in_days?(time_end_days)
    due_on && due_on >= Time.current.to_date && due_on < (Time.current+time_end_days.day).to_date
  end
  ...
  def get_visible_comments(current_user)
    if private?
      # Comments seen by it's posters, admins, and the creator of the task.
      comments.map{ |c| c if ((c.user == current_user) || c.project.admin?(current_user) || owner?(current_user) || owner?(c.user))}.compact | [comments.last]
    else
      comments
    end
  end
  ...
  #creates the calendar event related to the created task
  def create_calendar_event
    e = Event.new
    e.name = name
    if (start_date.nil? and due_on.nil?)
      e.start_at = e.end_at = nil
    elsif (start_date.nil?)
      e.start_at = e.end_at = due_on.to_datetime
    elsif (due_on.nil?)
      e.start_at = e.end_at = start_date.to_datetime
    else
      e.start_at = start_date.to_datetime
      e.end_at = due_on.to_datetime
    end
    e.task_id = id
    e.save
  end

  #updates the calendar event related to the task
  def update_calendar_event
    e = Event.where(:task_id => id).first
    if !e.nil?
      if deleted
        e.destroy
      else
  ...
  def has_participants
    TaskParticipant.where(:task_id => id).count > 0
  end

  def participants_logins
    logins = ''
    first = true
    task_participants = TaskParticipant.where(:task_id => id)
    task_participants.each do |tp|
      logins = logins.concat(',') if not first
      logins = logins.concat(User.find(tp.user_id).login)
      first = false
    end
    logins
  end

  def has_responsible_and_date(responsible, date, current_user_id)
    return false if status > 2
    if responsible != "1"
      if responsible == "2"
        person = Person.where(:user_id => current_user_id, :project_id => project_id).first
        return false if assigned_id != person.id
      elsif responsible == "3"
        return false if assigned_id != 0
      else
        filter_user_id = User.find_by_login(responsible).id
        person = Person.where(:user_id => filter_user_id, :project_id => project_id).first
        return false if assigned_id != person.id
      end
    end
    if date != "anytime" and date != "date"
      if date == "late_tasks"
        return false if not overdue?
      elsif date == "no_date_assigned"
        return false if not due_on.nil?
      elsif date == "today"
        return false if not due_today?
      elsif date == "tomorrow"
        return false if not due_tomorrow?
      elsif date == "week"
        return false if not due_in_days?(7)
      elsif date == "2weeks"
        return false if not due_in_days?(14)
      elsif date == "3weeks"
        return false if not due_in_days?(21)
      elsif date == "month"
        return false if not due_in_days?(30)
      end
    end
    true
  end
  ...
  def save_changes_to_comment # before_save
    ...
    if priority_changed? or self.new_record?
      comment.priority = self.priority
      comment.previous_priority = self.priority_was if priority_changed?
    end
    ...
    if start_date_changed? or self.new_record?
      comment.start_date = self.start_date
      comment.previous_start_date = self.start_date_was if start_date_changed?
    end

    if task_participants_changed? or self.new_record?
      comment.task_participants = self.task_participants
      comment.previous_task_participants = self.task_participants_was if task_participants_changed?
    end
