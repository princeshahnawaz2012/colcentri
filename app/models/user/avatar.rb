class User
  ...
  has_attached_file :avatar, 
    ...
    :path => (Colcentric.config.amazon_s3 ? "avatars/:id/:style.png" : ":rails_root/public/avatars/:id/:style.png"),
  ...
  def avatar_or_gravatar_url(size = :thumb, secure = false)
    avatar_or_gravatar_path(size, secure).tap do |url|
      if url.starts_with? 'http'
        ...
        scheme = secure ? 'https:' : 'http:'
        url.replace '%s//%s%s' % [scheme, Colcentric.config.app_domain, url]
      end
    end
  end
  ...
  private
    ...
    def gravatar_id
      Digest::MD5.hexdigest 'admin@kinubi.com'
    end

    def gravatar(size, secure = false, default = Colcentric.config.gravatar_default)
