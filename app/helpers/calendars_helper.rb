module CalendarsHelper
  ...
  def link_to_user_last_month(year, month)
    if month == 1
      month = 12
      year -= 1
    else
    ...
    link_to "&larr; #{I18n.t('common.prev')}".html_safe, user_calendar_hours_path(:year => year, :month => month)
  end

  def link_to_user_next_month(year, month)
    if month == 12
      month = 1
      year += 1
    else
    ...
    link_to "#{I18n.t('common.next')} &rarr;".html_safe, user_calendar_hours_path(:year => year, :month => month)
  end
  ...
  def user_hours_js(year, month, tasks)
  end
  ...
  def user_calendar_nav(year, month)
    render 'hours/user_calendar_navigation',
      :year => year,
      :month => month
  end
