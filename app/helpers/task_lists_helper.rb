module TaskListsHelper
  ...
  def task_list_link(project,task_list=nil)
    ...
    link_to image_tag(t('buttons.create_task_list')),
      list_link,
  ...
  def archive_task_list_link(project,task_list, on_index=false)
    link_to t('task_lists.actions.archive'),
            '#', :class => 'taskListResolve', :style => 'color: green; margin-left: 3px; text-decoration: none',
            :aconfirm => t('task_lists.actions.confirm_resolve_and_archive'),
            :action_url => archive_project_task_list_path(project, task_list, :on_index => (on_index ? 1 : 0))
  end
  ...
  def show_archived_tasks_link(project,task_list)
    ...
    show_task_id = 'show_closed_tasks_' + task_list.id.to_s
    link_to t('task_lists.actions.show_archived', :count => archived_tasks),
            project_task_lists_path(project, task_list),
            :class => 'show_archived_tasks_link', :style => 'color: green', :id => show_task_id, :onclick => 'Element.show("hide_closed_tasks_' + task_list.id.to_s + '");'
  end
  ...
  def responsible_code(responsible)
    if responsible == t('task_lists.filter.responsible') or responsible == t('task_lists.filter.anybody')
      "1"
    elsif responsible == t('task_lists.filter.my_tasks')
      "2"
    elsif responsible == t('task_lists.filter.unassigned')
      "3"
    else
      responsible
    end
  ...
  def date_code(date)
    if date == t('task_lists.filter.date')
      "date"
    elsif date == t('task_lists.filter.anytime')
      "anytime"
    elsif date == t('task_lists.filter.late_tasks')
      "late_tasks"
    elsif date == t('task_lists.filter.no_date_assigned')
      "no_date_assigned"
    elsif date == t('task_lists.filter.today')
      "today"
    elsif date == t('task_lists.filter.tomorrow')
      "tomorrow"
    elsif date == t('task_lists.filter.week')
      "week"
    elsif date == t('task_lists.filter.2weeks')
      "2weeks"
    elsif date == t('task_lists.filter.3weeks')
      "3weeks"
    elsif date == t('task_lists.filter.month')
      "month"
    end
  end
