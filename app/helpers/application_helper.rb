...
module ApplicationHelper
  ...
  def logo_image
    logo = @organization ? @organization.logo(:top) : "LogoColcentric1.png"
    image_tag(logo, :alt => "Kinubi")
  end
  ...
  def support_link
    if url = Colcentric.config.support_url
  ...
  def help_link
    if url = Colcentric.config.help_url
  ...
  def time_tracking_enabled?
    Colcentric.config.allow_time_tracking || false
  end
  ...
  def configure_this_organization
    if Colcentric.config.community && @community_role == :admin && @community_organization.description.blank? && params[:organization].nil?
  ...
  # collecting stats about Colcentric installations
  def tracking_code
    if Colcentric.config.tracking_enabled and Rails.env.production?
      fake_img = "http://colcentric.com/logo.png/#{request.host}"
...
BLOCK
  ...
  def mailbox_path(message)
    case message
      when MessageCopy
        show_received_message_path(message)
      when Message
        if message.sent?
          show_sent_message_path(message)
        else
          show_saved_message_path(message)
        end
    end
  end

  def mailbox_recipients(message)
    case message
      when MessageCopy #received message : show  author, recipients, exclude self
        result = message.user.login + ' ' + t('mail.mailbox.to') + ' ' +  message.recipients.map(&:login).to_sentence
      when Message #sent message : show recipients
        if message.sent?
          result = message.recipients.map(&:login).to_sentence
        else
          result = message.draft_recipients.map(&:login).to_sentence
        end
    end
